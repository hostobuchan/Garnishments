    ...
    : Entity
  {
      ...
      )
    {
      ...
      */
      ...
      else if (formatMarkerBytes[0] == (byte)0x49 && formatMarkerBytes[1] == (byte)0x49) // Little Endian
      {
          short Tiff = (short)(stream.ReadByte() | stream.ReadByte() << 8);
          if (Tiff == 42) // Tiff
          {
              return new TiffImage(stream);
          }
          else { return null; }
      }
      else if (formatMarkerBytes[0] == (byte)0x4D && formatMarkerBytes[1] == (byte)0x4D) // Big Endian
      {
          short Tiff = (short)(stream.ReadByte() << 8 | stream.ReadByte());
          if (Tiff == 42) // Tiff
          {
              return new TiffImage(stream);
          }
